...
class Chess(nn.Module):
    ...
    def forward(self, fen):
        ...
        max_index = torch.argmax(x_policy, dim=1)
        uci_move = self.index_to_move[max_index]
        if turn == "b":
            uci_move = self.reflect_uci(uci_move)
    ...
    def fen_to_vector(self, fen):
        ...
        print(f"{board_state_tensor.shape=}")
        print(f"{special_token_tensor.shape=}")
