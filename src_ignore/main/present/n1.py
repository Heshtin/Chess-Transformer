...
iteration = 0
existing_model_path = None #"../../runs/lichess_test_run/iters/state_dict_v30.pth"
...
class PolicyHead(nn.Module):
    def __init__(self, model_config):
        ...
        self.Sm_layer = nn.Linear(model_config.n_embd, model_config.n_possible_moves)   # Scaling matrix
        self.Vm = torch.randn(model_config.n_possible_moves, model_config.n_embd).to(device)     # Vm as a matrix, shape (1968, n)
        self.Wm = torch.randn(model_config.n_embd, model_config.n_possible_moves).to(device)
        self.Bv = torch.randn(model_config.n_possible_moves).to(device)
        self.final_layer = nn.Linear(model_config.n_embd, 1)
        self.fc = nn.Linear(model_config.n_embd, 1968)  # Fully connected layer to output a scalar

    def forward(self, cls_embedding, masked_indices=None):
        policy = F.softmax((torch.bmm((self.Sm_layer(cls_embedding).unsqueeze(1).transpose(1,2) * self.Vm.unsqueeze(0) + cls_embedding.unsqueeze(1)), self.Wm.unsqueeze(0).expand(batch_size, -1, -1)).sum(dim=2) + self.Bv), dim=0)
        
        
        # y = self.Sm_layer(cls_embedding).unsqueeze(1)
        # y = y.transpose(1,2)
        # z = y * self.Vm.unsqueeze(0)
        # Om = z + cls_embedding.unsqueeze(1)
        # Wm_expanded = self.Wm.unsqueeze(0).expand(batch_size, -1, -1)
        # x = torch.bmm(Om, Wm_expanded).sum(dim=2) + self.Bv
    ...
    #     z = y * self.Vm.unsqueeze(0)
    #     a = z + cls_embedding.unsqueeze(1)
    #     print(f"{a.shape=}")
    #     import sys; sys.exit(0)
    #     x = self.final_layer(a)

    #     #x = self.final_layer((self.Sm_layer(cls_embedding) * self.Vm) + cls_embedding).squeeze(1)
    #     policy = F.softmax(x.squeeze(1), dim=0).squeeze(2)
...
class ChessDataset(Dataset):
    def __init__(self, db_path, n_limit, split, n1=0.8, n2=0.1, transform=None):
        ...
        print(f"{self.data[0].dtype=}")
        print(f"{self.data[1].dtype=}")
        print(f"{self.data[2].dtype=}")
...
if run_training:
    ...
    train_loader = DataLoader(train_dataset, batch_size=gpu_batch_size, shuffle=True, num_workers=n_workers, drop_last=True)
if run_validation:
    val_dataset = ChessDataset(db_path, n_limit, 'val', n1=0.8, n2=0.1)
    val_loader = DataLoader(val_dataset, batch_size=gpu_batch_size, shuffle=False, num_workers=n_workers, drop_last=True)
if run_testing:
    test_dataset = ChessDataset(db_path, n_limit, 'test', n1=0.8, n2=0.1)
    test_loader = DataLoader(test_dataset, batch_size=gpu_batch_size, shuffle=False, num_workers=n_workers, drop_last=True)
