...
iteration = 0
existing_model_path = None#"../runs/full_TCEC_run_1/iters/state_dict_v139.pth"
...
class ChessDataset(Dataset):
    ...
    def get_dataframe(self, db_path):
        ...
        query = f"SELECT move_index, x FROM evaluations LIMIT {self.n_limit};" # WHERE n_pieces <= 12" #Change number of pieces per position
        cursor.execute(query)
        results = cursor.fetchall()
        df = pd.DataFrame(results, columns=['move_index', 'x'])
        df = df.drop_duplicates(subset=['move_index', 'x'], keep='first')
        print("fetched df")
        return df
...
policy_params = sum(p.numel() for p in model.policy_head.parameters() if p.requires_grad)
...
def training(model, train_loader, val_loader, optimizer, grad_accum_steps, device, run_config, model_path, log_path):
    ...
    for step in range(run_config.total_steps):
        ...
        for micro_step in range(grad_accum_steps):
            try:
                data, p = next(train_iter)
            except StopIteration:
                train_iter = iter(train_loader)
                data, p = next(train_iter)

            data, p = data.to(device), p.to(device)

            # Evaluate the loss
            with torch.autocast(device_type=device, dtype=torch.bfloat16):
                x_policy, loss = model(data, p)
