...
prev_iteration = None
existing_model_path = f"../runs/full_TCEC_run_1/iters/state_dict_v{str(prev_iteration)}.pth" if prev_iteration else None
...
if prev_iteration:
    ...
    with open(existing_model_path, "r") as file:
    ...
    final_step = int(last_line_split[0][5:]) + 1
    pretrained_data[0] = final_step
    pretrained_data[3] = prev_iteration
else:
    iteration = 0
...
class ChessDataset(Dataset):
    ...
    def get_dataframe(self, db_path):
        ...
        query = f"SELECT move_index, legal_move_indices, x, evaluation FROM evaluations LIMIT {self.n_limit};" # WHERE n_pieces <= 12" #Change number of pieces per position
