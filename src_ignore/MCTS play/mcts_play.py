...
class Node():
    global_node_storage = []
    def __init__(self, board, model, device, move_to_index, index_to_move, prob, reached_by_move=None):
        self.board = board
        self.total_value = 0
        self.n_visits = 0
        ...
        self.policy_vector = self.get_policy(self.board)
    ...
    def update_node_storage(self):
        for i, node_list in enumerate(Node.global_node_storage):
            if node_list[0].board == self.board:
    ...
    def rollout(self):
        ...
        while True:
            ...
            current_policy = self.get_policy(board_copy)
    ...
    def expand_node(self, policy_vector):
        ...
        for i, value in enumerate(policy_vector[0]):
            ...
            self.children.append(Node(board_copy, self.model, self.device, self.move_to_index, self.index_to_move, value, reached_by_move=uci_move))
...
with torch.no_grad():
    board = chess.Board()
    current_node = Node(board, model, device, move_to_index, index_to_move, 1) #fix so that there is no rollout at root node
    first_iteration = True
    while True:
        if first_iteration:
            uci_move, current_node = current_node(60.0)
            first_iteration = False
        else:
            uci_move, current_node = current_node(30.0)
        move = chess.Move.from_uci(uci_move)
