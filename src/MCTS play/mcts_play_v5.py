...
existing_model_path = "../runs/full_TCEC_run_1/iters/state_dict_v126.pth"
...
class Node():
    ...
    def run(self):
        if self.n_visits == 0:
            ...
            if self.evaluate_terminal_state(self.board) is not None:
                value = self.evaluate_terminal_state(self.board)
            else:
    ...
    def update_ucb(self):
        self.ucb = 1 + (self.total_value / self.n_visits) + self.c_puct * self.prob * (math.sqrt(self.n_total_visits) / (1 + self.n_visits))
    ...
    def get_policy_value(self, board):
        ...
        legal_indices = torch.tensor([self.move_to_index[move.uci()] for move in list(board.legal_moves)])
        ...
        if not board.turn:
            x_value = -x_value
        data, legal_indices = data.to("cpu"), legal_indices.to("cpu")
...
class ValueHead(nn.Module):
    def __init__(self, model_config):
        ...
        log_reduction = int(math.log2(model_config.n_embd)) - 3
        ...
        for i in range(model_config.n_blocks_valuehead):
            ...
            self.blocks.append(LinBlock(model_config, in_channels=inchannels, out_channels=outchannels))
...
with torch.no_grad():
    ...
    fen = "rnbqk1r1/ppp2pQp/4pn2/8/3Pp3/P1P5/2P2PPP/R1B1KBNR w KQk - 0 1"
    board = chess.Board()
    print(board)
    ...
    while True:
        ...
        root_node.prune_tree(keep_node=current_node)
        root_node = current_node
        print("value="+str(root_node.get_policy_value(root_node.board)[1]))
