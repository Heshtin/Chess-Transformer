...
iteration = 0
existing_model_path = None #"../../runs/lichess_run/iters/state_dict_v15.pth"
...
class PolicyHead(nn.Module):
    def __init__(self, model_config):
        super().__init__()
        self.fc1 = nn.Linear(model_config.n_embd, model_config.n_possible_moves) # Output full policy
        self.fc2 = nn.Linear(model_config.n_embd, model_config.n_possible_moves) # Output reevaluation policy
    ...
    def forward(self, x, forward_pass="first", masked_indices=None):
        ...
        if forward_pass == "first":
            x = self.fc1(x)
        elif forward_pass == "reevaluation":
            x = self.fc2(x)
...
class Transformer(nn.Module):
    def __init__(self, model_config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            se = nn.Parameter(torch.randn(model_config.n_embd)), #scaling embedding
    ...
    def forward(self, board_state_tensor, special_tokens_tensor, reevaluation_moves_tensor=None):
        ...
        if reevaluation_moves_tensor is not None:
            ...
            scaling_emb = prob_tensor.unsqueeze(-1) * self.transformer.se #(B, n_moves_reevaluate, n_embd)
            rank_indices = torch.arange(0, 3, device=device).unsqueeze(0).expand(B, -1)  # Shape: (B, 3)
            rank_emb = self.mre(rank_indices)
            move_emb = raw_move_emb + scaling_emb + rank_emb 
        else:
            moves_tensor = torch.full((B, self.model_config.n_moves_reevaluate), self.model_config.n_possible_moves)
...
class Chess(nn.Module):
    ...
    def forward(self, board_state_tensor, special_token_tensor, target_p_tensor=None,):
        # idx is of shape (B, T)
        x = self.transformer(board_state_tensor, special_token_tensor, forward_pass="first")
...
class ChessIterableDataset(IterableDataset):
    def __init__(self, db_path, split, n_limit, n1=0.8, n2=0.1):
    ...
    def data_generator(self):
        ...
        if self.split == 'train':
            query = f"SELECT board_state, special_tokens, next_move FROM chess_analysis LIMIT {train_limit};"
        elif self.split == 'val':
            query = f"SELECT board_state, special_tokens, next_move FROM chess_analysis LIMIT {val_limit} OFFSET {train_limit};"
        elif self.split == 'test':
            query = f"SELECT board_state, special_tokens, next_move FROM chess_analysis LIMIT {test_limit} OFFSET {train_limit + val_limit};"
        ...
        for row in cursor.fetchall():
            ...
            # legal_moves = json.loads(row[3])
            # moves_left = row[4]

            yield (torch.tensor(board_state, dtype=torch.int64),
                   torch.tensor(special_tokens, dtype=torch.int64),
                   torch.tensor(target_move_index, dtype=torch.int64))
...
if run_training:
    train_dataset = ChessIterableDataset(db_path, 'train', global_n_limit, n1=0.8, n2=0.1)
    train_loader = DataLoader(train_dataset, batch_size=gpu_batch_size, num_workers=n_workers)
if run_validation:
    val_dataset = ChessIterableDataset(db_path, 'val', global_n_limit, n1=0.8, n2=0.1)
    val_loader = DataLoader(val_dataset, batch_size=gpu_batch_size, num_workers=n_workers)
if run_testing:
    test_dataset = ChessIterableDataset(db_path, 'test', global_n_limit, n1=0.8, n2=0.1)
    test_loader = DataLoader(test_dataset, batch_size=gpu_batch_size, num_workers=n_workers)
    ...
    for step in range(run_config.total_steps):
        ...
        if step % 1000 == 0 or step == run_config.total_steps - 1:
            ...
            if log_path is not None:
                ...
                loss_storage = {}
        print(f"step={step} | loss={value[0]} | loss_p={value[1]} | loss_rp={value[2]}")
