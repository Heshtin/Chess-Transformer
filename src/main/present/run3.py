...
db_path = '/workspace/database/lichess_2/lichess_2.db'
...
iteration = 0
existing_model_path = "../../runs/lichess_test_run/iters/state_dict_v14.pth"
...
class ChessDataset(Dataset):
    ...
    # def save_tensors_to_disk(board_state_tensor, special_token_tensor, target_p_tensor, batch_num, output_dir):
    # torch.save(board_state_tensor, os.path.join(output_dir, f"board_state_tensor_{batch_num}.pt"))
    # torch.save(special_token_tensor, os.path.join(output_dir, f"special_token_tensor_{batch_num}.pt"))
    # torch.save(target_p_tensor, os.path.join(output_dir, f"target_p_tensor_{batch_num}.pt"))

        # Function to process chunk with optional sub-batching
    def process_chunk(self, df_chunk, sub_batch_size):
        board_state_list, special_token_list, target_p_list = [], [], []
        board_state_tensor, special_token_tensor, target_p_tensor = None, None, None

        rows_processed_in_chunk = 0  # Local counter for the current process

        for index, row in df_chunk.iterrows():
            # if rows_processed_in_chunk % 10000 == 0:
            #     print(f"each core processed {rows_processed_in_chunk} rows")
            ...
            # Process sub-batch when sub_batch_size is reached
            if len(board_state_list) == sub_batch_size:
                batch_board_state_tensor = torch.tensor(board_state_list, dtype=torch.int64)
                batch_special_token_tensor = torch.tensor(special_token_list, dtype=torch.int64)
                batch_target_p_tensor = torch.tensor(target_p_list, dtype=torch.float32)

                # Concatenate the sub-batch to final tensors
                if board_state_tensor is None:
                    board_state_tensor = batch_board_state_tensor.clone()
                    special_token_tensor = batch_special_token_tensor.clone()
                    target_p_tensor = batch_target_p_tensor.clone()
                else:
                    board_state_tensor = torch.cat((board_state_tensor, batch_board_state_tensor), dim=0)
                    special_token_tensor = torch.cat((special_token_tensor, batch_special_token_tensor), dim=0)
                    target_p_tensor = torch.cat((target_p_tensor, batch_target_p_tensor), dim=0)

                # Reset lists for the next sub-batch
                board_state_list, special_token_list, target_p_list = [], [], []

        # Process any remaining rows (if the chunk didn't divide evenly by sub_batch_size)
        if board_state_list:
            batch_board_state_tensor = torch.tensor(board_state_list, dtype=torch.int64)
            batch_special_token_tensor = torch.tensor(special_token_list, dtype=torch.int64)
            batch_target_p_tensor = torch.tensor(target_p_list, dtype=torch.float32)
    ...
    # Main function to convert df with multiprocessing and sub-batching
    def convert_df(self, start_index, end_index, num_processes=6, pool_chunk_size=100000, sub_batch_size=None):
        print("Converting df with multiprocessing and sub-batching")
        ...
        # If not using sub-batches, set sub_batch_size to the entire chunk size
        if sub_batch_size is None:
            sub_batch_size = pool_chunk_size

        df_chunks = [self.df.iloc[i:i + pool_chunk_size] for i in range(start_index, end_index, pool_chunk_size)]


        # Use multiprocessing Pool for parallel processing
        with Pool(processes=num_processes) as pool:
            results = pool.starmap(self.process_chunk, [(df_chunk, sub_batch_size) for df_chunk in df_chunks])


        # Combine results
        board_state_tensor, special_token_tensor, target_p_tensor = results[0]
        for batch_board_state_tensor, batch_special_token_tensor, batch_target_p_tensor in results[1:]:
            board_state_tensor = torch.cat((board_state_tensor, batch_board_state_tensor), dim=0)
            special_token_tensor = torch.cat((special_token_tensor, batch_special_token_tensor), dim=0)
            target_p_tensor = torch.cat((target_p_tensor, batch_target_p_tensor), dim=0)
