...
model_dir = "/workspace/runs/bay_optim_run_1/iters2"
best_model_path = "/workspace/runs/bay_optim_run_1/best_model_2.pth"

iteration = 0
existing_model_path = "../runs/bay_optim_run_1/iters2/state_dict_v18.pth"
pretrained_data = (1000, 4096, "TCEC_github_v2")

run_training = True
run_validation = True
run_testing = False

with open("/workspace/runs/bay_optim_run_1/completed_indices.txt", "a+") as file:
    file.seek(0)
...
class Chess(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        if existing_model_path is not None:
            ...
            self.load_state_dict(torch.load(existing_model_path))
        else:
...
with open(log_path, 'w') as log_file:
    if existing_model_path is not None:
        log_file.write(f"Pretrained model, for {pretrained_data[0]} steps with bs {pretrained_data[1]} using ds {pretrained_data[2]}")
...
def training(model, train_loader, optimizer, grad_accum_steps, device, run_config, model_path, log_path):
...
if run_training:
    training(model, train_loader, optimizer, grad_accum_steps, device, run_config, model_path, log_path)
