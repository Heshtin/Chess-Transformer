...
## Attention Patching
In the paper "Evidence of Learned Look-Ahead in a Chess-Playing Neural Network" (https://arxiv.org/pdf/2406.00877), a technique called activation patching is used to check the importance of each embedding to the ultimate output of the chess transformer. They analysed leela chess zero's model, which uses a slightly different approach to calculating the output. The transformer takes 64 tokens as input, 1 representing each square. Hence in the output each embedding corresponds to a specific square. For every possible start and end square (of a possible piece movement), the embeddings of those squares are used to calculate the logit for the corresponding uci move. The logits are then softmaxxed to produce the policy vector. It should be noted that leela chess zero used board flipping and piece colour swap when processing positions where it was black to move. As mentioned before, this prevents the model from learning how black pieces moves, hence severly impeding ability to look ahead. 

The paper used activation patching on leela chess zero when analysing certain positions. The technique involves "patching in" corrupted (slightly altered) activations for the embedding corresponding to a specific square, and then letting the forward pass continue as normal. The output was then analysed for significant deviations from the initial output. They found in their experiment that patching of the target square of either the 1st (immediate move) or 3rd move (move after opponent's reponse) significantly altered the output, showing that those embeddings were important to an accurate prediction. The importance of the square corresponding to the future 3rd move was evidence of look ahead by the engine. This contradicts our statement about how board flipping severly impedes look ahead. However, the authors also mentioned that patching of the target square of the 2nd move (opponent's immediate response) did not yield any significant change in the output, which seemed to indicaate that it had no significant effect on the model's prediction. This is clearly unexpected and the authors could not provide a reason for why it was so.

Our theory is that once the model learns how white pieces move, it implicity gets an idea of how black moves by seeing how white pieces move in response to black pieces. For example, it is generally not a good idea for white to put their rook on a diagonal that a black bishop is in (since the bishop can capture it). The model notices the lack of this and can interpret this to mean bishops have an influence diagonally. This is a weaker understanding than "the bishop can move diagonally and capture the rook" but it is an understanding nonetheless. Hence, learning the dynamics of black pieces is likely much slower but still possible. Thus while the model has some idea of how the black pieces move it is very weak resulting in weak implicit search. This is not a problem in a model like llea chess zero that uses explicit search, but will severly hinder performance that do not use explicit search. This line of reasoning could also explain why patching the target square of the 2nd move did not elicit a significant change in output policy, since the understanding of how black would move in response is much weaker. We intend to recreate a similar experiment using our model and test our hypothesis.
